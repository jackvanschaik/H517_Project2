...
function draw_ali_plots(data) {
  ...
  data.forEach((element) => {
    if (element.CARRIER_NAME in airport) {
      airport[element.CARRIER_NAME].delay_values +=
        Number(element.DEP_DEL15) || 0;

      airport[element.CARRIER_NAME].age[element.PLANE_AGE] =
        airport[element.CARRIER_NAME].age[element.PLANE_AGE] + 1 || 1;
    } else {
  ...
  var del_max = d3.max(
    Object.entries(airport).map(function (x) {
      return x[1]["delay_values"];
    })
  );

  var y = d3.scaleLinear().domain([0, del_max]).range([height, 0]);
  ...
  svg
    ...
    .attr("y", function (d) {
      return y(airport[d]["delay_values"]);
    })
    .attr("width", x.bandwidth())
    .attr("height", function (d) {
      return height - y(airport[d]["delay_values"]);
    })
    .attr("fill", "#69b3a2")
    ...
    .on("mouseover", function (d, i) {
      d3.selectAll("." + d.replace(/\s+/g, "").replace(".", "")).attr(
        "fill",
        "yellow"
      );
    })
    .on("mouseout", function (d, i) {
      d3.select("rect." + d.replace(/\s+/g, "").replace(".", "")).attr(
        "fill",
        "#69b3a2"
      );
      d3.select("path." + d.replace(/\s+/g, "").replace(".", "")).attr(
        "fill",
        color(airport[d].planesOver25)
      );
    });
  ...
  arc
    ...
    .on("mouseover", function (d, i) {
      d3.selectAll("." + d.data.name.replace(/\s+/g, "").replace(".", "")).attr(
        "fill",
        "yellow"
      );
    })
    .on("mouseout", function (d, i) {
      d3.select(
        "rect." + d.data.name.replace(/\s+/g, "").replace(".", "")
      ).attr("fill", "#69b3a2");
      d3.select(
        "path." + d.data.name.replace(/\s+/g, "").replace(".", "")
...
function draw_map(data) {
  ...
  svg
    ...
    .on("mouseout", function () {
      ...
      d3.select(this).attr("r", 3).attr("fill", "brown").style("opacity", 0.3);
    })
    .on("click", function (d) {
      window["airport"] = d[2];
...
/*** UI Reactivity ***/
function make_ui() {
  d3.select("#delayonly").on("change", function (d) {
    visualize();
  });
  d3.select("#depblock").on("change", function (d) {
    visualize();
  });
  d3.select("#dow").on("change", function (d) {
    visualize();
  });

  d3.select("#textsearch").on("keyup", function (d) {
    search_airports();
  });
}
